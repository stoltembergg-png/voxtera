...
impl Server {
    ...
    pub fn tick(&mut self, _input: Input, dt: Duration) -> Result<Vec<Event>, Error> {
        ...
        // Update calendar events when the refresh interval expires. Calendar events
        // change on day boundaries, so rebuilding this value every simulation tick
        // only adds clock calls and allocations to the hot path.
        let now = Instant::now();
